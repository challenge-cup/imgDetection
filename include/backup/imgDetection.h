...
public:
	...
	void getNumCenter(cv::Mat &_img, cv::Point &centerP, std::vector<cv::Point> &P, bool &state);			//获取数字中心
	void getMultiNumCenter(cv::Mat &_img, cv::Point &centerP, std::vector<cv::Point> &P, bool &state);
	...
	void getBoradDirection(cv::Mat &_img, int &numDirect, bool &state);                //检测标志牌方向
